...
动画和渲染由[SKView]()的对象执行。将这个SKView的对象添加到窗口中，然后由它来绘制内容。由于SKView的对象是一个view，所以它可以与其他view通过view的层级结构进行组合。
...
[SKScene]()类是[SKNode]()类的子类的子类（SKScene:SKEffectNode:SKNode）。当使用SpriteKit时，节点用来将所有内容组织起来，场景对象作为节点树中其他节点的根节点，场景对象决定绘制内容和如何渲染。
